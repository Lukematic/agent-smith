...
def kilo_project_drift(awino_home: Path, project: Path) -> list[str]:
    ...
    expected = _persona_for(Harness.KILO, awino_home / "agents" / "awino.md")
...
def install(
    awino_home: Path, target: Target, *, skills: bool = True, overwrite: bool = False
) -> list[Action]:
    ...
    if not target.harness.installs_persona_file:
        ...
        persona_source = awino_home / "agents" / "awino.md"
...
def skill_drift(awino_home: Path, target: Target) -> list[SkillDrift]:
...
def refresh_skills(awino_home: Path, target: Target) -> list[Action]:
    ...
    label = f"{target.harness}/{target.scope}"
    actions: list[Action] = []
    for item in skill_drift(awino_home, target):
        ...
        if item.state == "human-modified":
            ...
            continue
        source = awino_home / "skills" / item.skill
        if item.path.exists():
...
def pointer_text(awino_home: Path) -> str:
