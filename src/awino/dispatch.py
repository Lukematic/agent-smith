...
def _skill_text(awino_home: Path, skill: str) -> str:
    path = awino_home / "skills" / skill / "SKILL.md"
    if path.is_file():
        return path.read_text(encoding="utf-8")
    return f"(skill file not found at {path})"


def _payload_home(payload: dict) -> Path:
    """A.W.I.N.O. home recorded in a floor payload.

    Written as ``"awino_home"`` since the rename; ``"smith_home"`` payloads
    written before it still resume.
    """
    return Path(str(payload.get("awino_home", payload.get("smith_home"))))
...
def _write_review_prompt(
    ...
    prompt = (
        ...
        + "\n\n## Reviewer procedure\n\n"
        + _skill_text(awino_home, "awino-consult")
    )
    scratch = ledger.state_root / "assignments"
    scratch.mkdir(parents=True, exist_ok=True)
    prompt_path = scratch / f"{invocation_id}.md"
...
def close_floor(ledger: Ledger, run_id: str, project: Path) -> FloorResult:
    ...
    try:
        from awino.provision import ensure_project_venv, project_env
