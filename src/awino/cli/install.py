...
def install_command(
    ...
    awino_home = workspace.home.root
    ...
    _echo(f"A.W.I.N.O. home: {awino_home}")
    ...
    for target in chosen:
        ...
        if dry_run:
            ...
            continue
        for action in harness.install(awino_home, target, overwrite=overwrite):
    ...
    if pointer and not dry_run:
        ...
        for name in ("AGENTS.md", "CLAUDE.md"):
            ...
            if any(marker in body for marker in markers):
                ...
                continue
            path.write_text(
                body.rstrip() + "\n\n" + harness.pointer_text(awino_home), encoding="utf-8"
            )
            _echo(f"POINTER    appended to {name}")
            break
        else:
            target_file = workspace.project.root / "AGENTS.md"
            target_file.write_text(harness.pointer_text(awino_home), encoding="utf-8")
    ...
    if scope_wanted == "project" and (which is None or which == "kilo"):
        for action in harness.repair_kilo_project(awino_home, workspace.project.root):
            _echo(f"  {action.outcome:<10} {action.path.name}  {action.detail}")
...
def install_mode_command(
    ...
    if emit_json:
        _echo(modes.as_json(awino_home))
        return
    ...
    built = modes.build_modes(awino_home)
...
def skills_status_command(
    ...
    if as_json_output:
        payload = {
            f"{t.harness}/{t.scope}": [
                ...
                for d in harness.skill_drift(awino_home, t)
            ]
            for t in targets
        }
    ...
    total_drifted = 0
    for target in targets:
        drift = harness.skill_drift(awino_home, target)
...
def install_refresh_command(
    ...
    any_failed = False
    for target in targets:
        for action in harness.refresh_skills(awino_home, target):
            _echo(f"{action.outcome:<10} {action.path}  {action.detail}")
