...
def mission_command(
    ...
    verification commands, and derived insights written to .awino/MISSION.md.
    """
    workspace = _workspace()
    tracker = seeds.Seeds(workspace.project.root)
...
def best_command(
    ...
    if request is None:
        ...
        from awino.cli.buddy import buddy_check as _buddy_check

        _buddy_check(fix=False)
        _echo("")
        from awino import machine as _m
...
def start_command(
    ...
    from awino import harness, updater

    # The repository that A.W.I.N.O. serves need not be a Python project. Its
    # installation at `<project>/.awino` already owns a declared, locked Python
