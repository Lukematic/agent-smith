...
def check_skill_registry(paths: AwinoPaths) -> Result:
...
def check_clone_freshness(paths: AwinoPaths) -> Result:
...
def check_uv(paths: AwinoPaths) -> Result:
...
def check_python_pin(paths: AwinoPaths) -> Result:
...
def check_justfile_shim(paths: AwinoPaths) -> Result:
...
def check_pyproject(paths: AwinoPaths) -> Result:
...
def check_lint(paths: AwinoPaths) -> Result:
    code, out = run_command(
...
def check_format(paths: AwinoPaths) -> Result:
...
def check_tests(paths: AwinoPaths) -> Result:
...
def check_docs(paths: AwinoPaths) -> Result:
...
def check_artifacts(paths: AwinoPaths) -> Result:
    """Every authored skill and agent must pass the validator."""
    from awino.validate import discover, validate_file
...
def check_module_size(paths: AwinoPaths) -> Result:
    ...
    """
    package = paths.root / "src" / "awino"
    if not package.is_dir():
        return _ok("module_size", "no src/awino package to measure, skipping")
    ...
    if oversized:
        ...
        return _warn(
            ...
            "split along command or responsibility seams; see src/awino/cli/ for the pattern",
        )
...
def check_folder_docs(paths: AwinoPaths) -> Result:
...
def check_capability_claims(paths: AwinoPaths) -> Result:
...
def check_seeds(paths: AwinoPaths, project_root: Path | None = None) -> Result:
    ...
    """
    from awino.seeds import Seeds, SeedsState
...
def check_memory(paths: AwinoPaths) -> Result:
...
def check_knowledge(paths: AwinoPaths) -> Result:
...
def check_harness_install(_paths: AwinoPaths) -> Result:
...
FAST_CHECKS: tuple[Callable[[AwinoPaths], Result], ...] = (
    check_uv,
    check_python_pin,
    check_justfile_shim,
    check_pyproject,
...
SLOW_CHECKS: tuple[Callable[[AwinoPaths], Result], ...] = (
    check_lint,
    check_format,
    check_tests,
)
...
def run_all(paths: AwinoPaths, *, fast: bool = False) -> list[Result]:
...
def check_release_drift(paths: AwinoPaths) -> Result:
