...
def probe_knowledge(paths: AwinoPaths) -> Capability:
...
def probe_gate(_paths: AwinoPaths) -> Capability:
    from awino.enforce import CONTRACTS
...
def probe_spawn(_paths: AwinoPaths) -> Capability:
    ...
    """
    from awino.spawn import detect_runner
...
def probe_self_repair(_paths: AwinoPaths) -> Capability:
    from awino import fix
...
def probe_memory(paths: AwinoPaths) -> Capability:
    import re
    ...
    if paths.is_bundled:
        return _degraded(
            "remember lessons across sessions",
            f"{count} lessons",
            "installed as a wheel, so new lessons go to ~/.awino and not the bundle",
        )
    return _real("remember lessons across sessions", f"{count} dated lessons, append-only")


def probe_self_improvement(_paths: AwinoPaths) -> Capability:
...
def probe_autonomy(paths: AwinoPaths) -> Capability:
...
def probe_toolchain(_paths: AwinoPaths) -> Capability:
    from awino.paths import Workspace
    from awino.toolchain import Toolchain
...
def probe_harness_install(_paths: AwinoPaths) -> Capability:
    from awino.harness import status
    from awino.paths import Workspace
...
def probe_tracker(_paths: AwinoPaths) -> Capability:
    from awino.paths import Workspace
...
def probe_diagrams(_paths: AwinoPaths) -> Capability:
...
def probe_network(_paths: AwinoPaths) -> Capability:
...
PROBES: tuple[Callable[[AwinoPaths], Capability], ...] = (
    probe_knowledge,
    probe_gate,
    probe_memory,
    probe_spawn,
...
def assess(paths: AwinoPaths | None = None) -> list[Capability]:
    resolved = paths or AwinoPaths.discover()
    out: list[Capability] = []
    for probe in PROBES:
        try:
...
CLAIMED_IN_DOCS: dict[str, Callable[[AwinoPaths], Capability]] = {
...
def audit_claims(paths: AwinoPaths | None = None) -> list[tuple[str, Capability]]:
