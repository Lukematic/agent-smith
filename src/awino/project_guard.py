...
from awino.onboarding import ProjectIntent
...
def _debug_edit_decision(payload: dict, project: Path) -> dict | None:
    if payload.get("tool_name") not in {"Edit", "Write", "MultiEdit"}:
        return None
    awino_dir = project_state_dir(project)
    state_root = awino_dir / "state" if (awino_dir / "plugin.json").is_file() else awino_dir
