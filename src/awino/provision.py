...
def plan(project: Path, state_root: Path | None = None) -> list[Step]:
    ...
    state_dir = state_root if state_root is not None else project_state_dir(project)
    ...
    *,
    ...
    for step in steps:
        ...
        if step.kind is StepKind.SCAFFOLD_STATE:
            project_state_dir(project).joinpath("run").mkdir(parents=True, exist_ok=True)
            project_state_dir(project).joinpath("memory").mkdir(parents=True, exist_ok=True)
            actions.append(Action(step.kind, "CREATED", ".awino/{run,memory}"))
            continue
