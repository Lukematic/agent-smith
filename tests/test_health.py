...
@pytest.fixture
def blank(tmp_path: Path) -> AwinoPaths:
...
class TestRealProjectIsHealthy:
    ...
    def test_no_failing_gates(self, real: AwinoPaths) -> None:
    ...
    def test_docs_are_all_linked(self, real: AwinoPaths) -> None:
        assert check_docs(real).health is Health.OK

    def test_memory_ledger_is_well_formed(self, real: AwinoPaths) -> None:
        assert check_memory(real).health is Health.OK

    def test_pyproject_configures_quality_tools(self, real: AwinoPaths) -> None:
        assert check_pyproject(real).health is Health.OK


class TestDocsGate:
    def test_missing_readme_fails(self, blank: AwinoPaths) -> None:
        assert check_docs(blank).health is Health.FAIL

    def test_unlinked_doc_fails(self, blank: AwinoPaths) -> None:
    ...
    def test_linked_doc_passes(self, blank: AwinoPaths) -> None:
    ...
    def test_broken_link_fails(self, blank: AwinoPaths) -> None:
...
class TestMemoryGate:
    def test_missing_ledger_fails(self, blank: AwinoPaths) -> None:
        assert check_memory(blank).health is Health.FAIL

    def test_undated_lesson_fails(self, blank: AwinoPaths) -> None:
        blank.lessons.write_text("# Lessons\n\n- a rule with no date\n", encoding="utf-8")
        assert check_memory(blank).health is Health.FAIL

    def test_dated_lesson_passes(self, blank: AwinoPaths) -> None:
        blank.lessons.write_text(
            "# Lessons\n\n- [2026-08-21] `MODE` do the thing. (surface: tools)\n", encoding="utf-8"
        )
        assert check_memory(blank).health is Health.OK

    def test_format_template_in_a_fence_is_not_a_lesson(self, blank: AwinoPaths) -> None:
    ...
    def test_empty_ledger_warns_without_blocking(self, blank: AwinoPaths) -> None:
        blank.lessons.write_text("# Lessons\n\nNo rules yet.\n", encoding="utf-8")
...
class TestPyprojectGate:
    def test_missing_pyproject_fails(self, blank: AwinoPaths) -> None:
        assert check_pyproject(blank).health is Health.FAIL

    def test_unparseable_pyproject_fails(self, blank: AwinoPaths) -> None:
        (blank.root / "pyproject.toml").write_text("[project\nbroken", encoding="utf-8")
        result = check_pyproject(blank)
        assert result.health is Health.FAIL
        assert "parse" in result.detail

    def test_pyproject_without_ruff_fails(self, blank: AwinoPaths) -> None:
    ...
    def test_ruff_missing_core_rules_fails(self, blank: AwinoPaths) -> None:
        (blank.root / "pyproject.toml").write_text(
            '[project]\nname = "x"\nversion = "1"\nrequires-python = ">=3.12"\n'
            '[tool.ruff.lint]\nselect = ["W"]\n'
...
class TestJustfileGate:
    def test_missing_justfile_fails(self, blank: AwinoPaths) -> None:
        assert check_justfile_shim(blank).health is Health.FAIL

    def test_justfile_missing_required_recipes_fails(self, blank: AwinoPaths) -> None:
...
class TestReporting:
    def test_summary_counts_every_result(self, real: AwinoPaths) -> None:
        results = health.run_all(real, fast=True)
        counts = health.summarise(results)
        assert sum(counts.values()) == len(results)

    def test_json_output_is_valid(self, real: AwinoPaths) -> None:
        import json
    ...
    def test_every_non_ok_result_carries_a_remedy(self, real: AwinoPaths) -> None:
...
class TestProjectAwareSeeds:
    def test_project_root_does_not_inherit_awino_homes_tracker(
        self, blank: AwinoPaths, tmp_path: Path
    ) -> None:
    ...
    def test_legacy_markdown_tracker_warns_instead_of_demanding_deletion(
        self, blank: AwinoPaths, tmp_path: Path, monkeypatch
    ) -> None:
        from awino import seeds as seeds_module
...
class TestCloneFreshness:
    ...
    def _clone_with_remote(self, tmp_path: Path) -> AwinoPaths:
    ...
    def test_non_git_directory_is_ok_not_a_failure(self, blank: AwinoPaths) -> None:
