...
def test_legacy_persona_is_not_exposed_as_a_skill(tmp_path: Path) -> None:
    roots = [tmp_path / name for name in ("project", "global", "bundled")]
    write_skill(roots[0], "agent-smith", "Agentic engineering persona")
    write_skill(roots[0], "awino-rpi", "research plan implementation")
    catalog = SkillCatalog(*roots)
    assert [skill.name for skill in catalog.skills] == ["awino-rpi"]
