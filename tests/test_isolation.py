...
def test_session_logs_do_not_cross_projects(tmp_path: Path) -> None:
    ...
    a_text = "".join(p.read_text(encoding="utf-8") for p in (a / ".awino").rglob("*.jsonl"))
    b_text = "".join(p.read_text(encoding="utf-8") for p in (b / ".awino").rglob("*.jsonl"))
...
def test_intent_and_ledger_do_not_cross_projects(tmp_path: Path) -> None:
    ...
    assert (a / ".awino" / "intent.json").is_file()
    assert not (b / ".awino" / "intent.json").exists()
...
def test_mission_documents_are_per_project(tmp_path: Path) -> None:
    ...
    assert "alpha mission text" in (a / ".awino" / "MISSION.md").read_text(encoding="utf-8")
    assert "alpha mission text" not in (b / ".awino" / "MISSION.md").read_text(encoding="utf-8")
