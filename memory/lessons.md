...
- [2026-09-10] `URL_DROP_BOOTSTRAP` — when given the A.W.I.N.O. GitHub URL (`https://github.com/Lukematic/agent-smith`) in a new project or session, the agent must check if A.W.I.N.O. is already installed globally (`awino --version` or `~/.local/bin/awino.ps1`), update if behind (`awino update`), and run `awino onboard` for the target project; it must never clone a second private copy of A.W.I.N.O. into the project or manually improvise scaffolding. (surface: prompt + context)
...
- [2026-09-04] `PROOF_OF_SPINE_IS_NOT_PROOF_OF_PAYOFF` - I live-proved locate->gates and declared the machine working while CLOSE only printed 'run gate close' and nothing called stance, mission, walkthrough or grill. Prove the node the human cares about (CLOSE) end to end, not just the routing spine.
- [2026-09-04] `SHELL_ALIAS_COLLISION` - a PowerShell helper named R resolved to the built-in alias r (Invoke-History) and re-ran a prior command, corrupting four docs (l->o). Never name helpers with single letters; verify a batch edit's diff before the next batch.
- [2026-09-05] `INHERITED_VENV` - gate commands launched via 'uv run --project <awino>' inherited awino's venv; labthing's lint ran awino's ruff and struck out three times for the wrong reason. Ledger.record now scrubs the inherited venv, prefers the target's .venv, and creates it when pyproject declares one. Three strikes on the wrong interpreter are not three strikes on the code.
