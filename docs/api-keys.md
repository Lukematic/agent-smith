...
Where A.W.I.N.O.'s subagent spawning looks for credentials, and how to point it at a
...
are a trust boundary. A.W.I.N.O. never reads, stores, or transmits them on your behalf;
...
`awino heal` and `awino delegate` diagnose it as `AUTH_MISSING` and refuse to guess
...
A.W.I.N.O. does not read your `.env` file and does not translate `LLM_*` variable names
...
- **`awino limits`** reports this honestly:
  ...
  (`available` checks `PATH`, not login state) — `awino delegate` is what catches
...
- **Adding a fourth runner is a real, trackable piece of work**, not a one-line
  ...
  a `awino gate open authoring` task rather than expecting `.env` autodetection to
  appear silently.
