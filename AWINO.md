...
```bash
awino limits              # what is REAL, DEGRADED, or ABSENT right now
awino limits --claims     # audit documented claims against reality
```
...
awino ladder      # what you should be authoring at each rung
```
...
```bash
awino delegate plan.json --dry-run    # writes prompts, plans waves, spawns nothing
awino delegate plan.json
```
...
awino tidy              # archive it, dated
awino clean             # remove only regenerable artifacts
awino fix               # repair what is mechanical, report what needs judgement
```
...
```bash
awino heal "<the failing command>"     # diagnose -> remedy -> retry, max 3 attempts
awino delegate plan.json                # spawn failures are diagnosed inline
```
...
reported with the exact human action — A.W.I.N.O. does not touch your credentials and
