...
description: Use on A.W.I.N.O. first run to create the .awino folder scaffold and verify the knowledge registry against upstream. Triggers on "bootstrap A.W.I.N.O.", "set up A.W.I.N.O.", missing .awino folder.
...
Check for `.awino/` at the workspace root.

- **Present** → skip to Step 3 (verification only).
- **Absent** → create the full scaffold in Step 2.

Never create `.awino/` in a subdirectory. It is a root-level sibling of
...
`AwinoPaths.discover()` against this scaffold — there is no `scripts/` folder
...
| Mode | Guard |
| --- | --- |
| `SCAFFOLD_IN_WRONG_PLACE` | .awino must be at workspace root |
